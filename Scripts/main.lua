...
if not _G.__RSDWRandomEvents_CleanupRegistered then
    ...
    RegisterKeyBind(Config.HotkeyCleanup, function()
        ExecuteInGameThread(function()
            ...
            local cathanClass = shortClassName(Config.CathanClassPath)
            local postiePeteClass = shortClassName(Config.PostiePeteClassPath)
            ...
            Encounter.CleanupNamed(ctx, cathanClass, "Cathan")
            Encounter.CleanupNamed(ctx, postiePeteClass, "Postie Pete")
            ...
            Encounter.CleanupNearest(ctx, cathanClass)
            Encounter.CleanupNearest(ctx, postiePeteClass)
        end)
    end)
else
